...
import type { PersonalizedInsights, RiskLevel } from "@/lib/types";
...
export function RiskProjection({
  insights,
}: {
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#1f1f24]">
                <th className="text-left py-3 pr-4 text-[#71717a] font-medium">
                  Category
                </th>
                <th className="text-center py-3 px-3 text-[#71717a] font-medium">
                  Now
                </th>
                <th className="text-center py-3 px-3 text-[#71717a] font-medium">
                  +5 yr
            ...
            <tbody>
              {insights.riskProjection.map((row, i) => (
                <tr key={i} className="border-b border-[#1f1f24]/50">
                  <td className="py-4 pr-4 font-medium text-[#d4d4d8]">
                    {row.category}
                  </td>
                  <RiskCell level={row.now.level} reason={row.now.reason} />
                  <RiskCell
                    level={row.fiveYear.level}
                    reason={row.fiveYear.reason}
                  />
                  <RiskCell
                    level={row.tenYear.level}
                    reason={row.tenYear.reason}
                  />
                </tr>
              ))}
            </tbody>
      ...
      {insights.recommendations.length > 0 && (
        <section className="card p-6">
          <h2 className="font-semibold mb-3 flex items-center gap-2">
            <ArrowRight className="w-4 h-4 text-[#7c5cff]" /> Coverage to
            consider
          </h2>
          <ul className="space-y-2">
            {insights.recommendations.map((r, i) => (
              <li key={i} className="text-sm text-[#d4d4d8] flex gap-2">
                <span className="text-[#7c5cff] mt-0.5 shrink-0">
                  &#x2192;
                </span>
                {r}
              </li>
